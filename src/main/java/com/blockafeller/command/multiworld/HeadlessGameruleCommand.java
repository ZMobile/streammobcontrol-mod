...
public class HeadlessGameruleCommand {

    @SuppressWarnings("rawtypes")
    public static HashMap<String, GameRules.Key> keys = new HashMap<>();

    @SuppressWarnings("unchecked")
    public static int run(MinecraftServer mc, String worldId, String[] args) {
        ServerWorld w = WorldByIdQuery.getWorldById(mc, worldId);
        ...
        /*if (args.length < 3) {
            GameRules.Rule<?> rule = w.getGameRules().get(keys.get(args[1]));
        ...
        if (is_bol) {
            // Boolean Rule
            GameRules.BooleanRule rule = (GameRules.BooleanRule) w.getGameRules().get(keys.get(a1));
            rule.set(Boolean.valueOf(a2), mc);
        } else {
            // Int Rule
            GameRules.IntRule rule = (GameRules.IntRule) w.getGameRules().get(keys.get(a1));
    ...
    public static void set_gamerule_from_cfg(ServerWorld world, String key, String val) {
        ...
        if (is_bol) {
            // Boolean Rule
            GameRules.BooleanRule rule = (GameRules.BooleanRule) world.getGameRules().get(keys.get(name));
            rule.set(Boolean.valueOf(a1), MultiworldMod.mc);
        } else {
            // Int Rule
            GameRules.IntRule rule = (GameRules.IntRule) world.getGameRules().get(keys.get(name));
